...
try {
    // Count total customers (assuming users table contains customers)
    $count_customers = $conn->prepare("SELECT COUNT(*) FROM `users`");
    $count_customers->execute();
    $happy_customers = $count_customers->fetchColumn();
    
    // Count total products available
    $count_products = $conn->prepare("SELECT SUM(stock) FROM `products`");
    $count_products->execute();
    $products_available = $count_products->fetchColumn();
    
    // Count delivered orders - FIXED
    $count_orders = $conn->prepare("SELECT COUNT(*) FROM `orders` WHERE status = 'pending'");
    $count_orders->execute();
    $orders_delivered = $count_orders->fetchColumn();
...
<section class="stats">
   <div class="box-container">
      <div class="box animate__animated" data-animate="fadeInUp">
         ...
         <h3 class="count" data-count="<?php echo $happy_customers; ?>">0</h3>
         <p>Happy Customers</p>
      </div>
      
      <div class="box animate__animated" data-animate="fadeInUp" data-delay="0.2s">
         <i class="fas fa-box-open"></i>
         <h3 class="count" data-count="<?php echo $products_available; ?>">0</h3>
         <p>Products Available</p>
      </div>
      
      <div class="box animate__animated" data-animate="fadeInUp" data-delay="0.4s">
         <i class="fas fa-truck"></i>
         <h3 class="count" data-count="<?php echo $orders_delivered; ?>">0</h3>
